...
* ユーザーがクリックしたすべて **太字**. 例：「Click **続行**.」と入力します。 オプション値とエラーメッセージの形式は次のとおりです。 _斜体_.
* 可能な限り、指示内で UI 要素タイプを記述しないでください。 （クリック **次**. とクリック **次** ボタン）。
* コマンド シーケンスで&quot;Choose&quot;と&quot;>&quot;を使用します。 （選択 **編集** > **環境設定**. とクリック編集 |環境設定。）
* 前置：ダイアログボックス、ウィンドウ、領域、パネル、表示、ウィザード、リスト、フォルダー、ノードの「in」。
* 前置位置：画面、ページ、ツールバー、メニューバー、タブ、ペイン、リボンの「オン」
* 前置位置：クリック（クリック） **次** 対クリック **次**）に設定します。
...
エラーメッセージ： _「異常が発生しました。」_

エラーメッセージ： __エラーが発生しました。__

</td>

<td colspan="1"> エラーメッセージ：  <i>エラーが発生しました。</i> </td>
...
* テキスト以外の要素またはグラフィック要素には、代替テキストまたは代替テキストが含まれます。 例： ![example_image](/url "alt_text_for_this_image").

* すべてのリンクは、リンクの宛先を記述します。 例 [リンク](/uri "destination_of_the_link").
...
外部のページにリンクするには、goURL ではなく通常の URL を使用します  [Adobe.com](http://Adobe.com).
...
* 常にカンマ （前にカンマ）を使用する _および_ または _または_ （3 つ以上の項目のリスト）:x、y、z
...
* Magento CommerceはAdobe Commerceになりました。 を参照してください [用語のリブランディング](https://github.com/magento/knowledge-base/wiki) 最新のブランディング言語の使用方法について詳しくは、wiki を参照してください。
