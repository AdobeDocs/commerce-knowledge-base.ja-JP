...
1. [[!UICONTROL indexers]](https://experienceleague.adobe.com/en/docs/commerce-operations/configuration-guide/cli/manage-indexers) （開発ドキュメント）が実行されており、```php    Product Feed    ``` [!UICONTROL indexer]が```php    Update by Schedule    ```に設定されていることを確認してください。
