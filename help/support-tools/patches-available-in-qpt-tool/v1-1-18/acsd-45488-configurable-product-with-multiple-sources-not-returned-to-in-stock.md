...
ACSD-45488 パッチを使用すると、複数のソースを持つ設定可能な製品が在庫として自動的に返されない問題を解決できます。 このパッチは、 [品質向上パッチツール（QPT）](/help/announcements/adobe-commerce-announcements/magento-quality-patches-released-new-tool-to-self-serve-quality-patches.md) 1.1.18 がインストールされています。 パッチ ID は ACSD-45488 です。 この問題はAdobe Commerce 2.4.6 で修正される予定であることに注意してください。
...
1. を確認します `stock_status` から `cataloginventory_stock_status`.
1. 関連する両方の製品を次のように設定します *在庫切れ*.
1. を確認します `stock_status` から `cataloginventory_stock_status`.
1. 両方の関連製品をに設定 *在庫あり*.
1. を確認します `stock_status` から `cataloginventory_stock_status`.
...
設定可能な商品の在庫ステータスが次のように更新されます *在庫あり* 関連付けられた製品が在庫に設定されている場合。

<u>実際の結果</u>:

設定可能な商品の在庫ステータスが「」に更新されません *在庫あり* 関連付けられた製品が在庫に設定されている場合。
