---
title: 「MDVA-41215:「mage-messages」 cookie を設定すると、ユーザーに 500 エラーが発生する」
description: MDVA-41215 パッチでは、「mage-messages」 cookie がすでに存在するが、新しいメッセージがない場合に、それを設定した後に 500 エラーが発生する問題を修正します。 このパッチは、[Quality Patches Tool （QPT） ] （/help/announcements/adobe-commerce-announcements/magento-quality-patches-released-new-tool-to-self-serve-quality-patches.md） 1.1.8 がインストールされている場合に利用できます。 パッチ ID は MDVA-41215。 この問題はAdobe Commerce 2.4.4 で修正される予定であることに注意してください。
...
# MDVA-41215:「mage-messages」 cookie を設定すると、500 エラーが発生する

MDVA-41215 パッチでは、「mage-messages」 cookie がすでに存在するが、新しいメッセージがない場合に、それを設定した後に 500 エラーが発生する問題を修正します。 このパッチは、 [品質向上パッチツール（QPT）](/help/announcements/adobe-commerce-announcements/magento-quality-patches-released-new-tool-to-self-serve-quality-patches.md) 1.1.8 がインストールされています。 パッチ ID は MDVA-41215。 この問題はAdobe Commerce 2.4.4 で修正される予定であることに注意してください。
...
「mage-messages」 cookie が既に存在するが、新しいメッセージがない場合、ユーザーが設定すると 500 エラーが発生します。

<u>再現手順</u>:

1. に移動 **../customer/account/login**.
1. ブラウザー開発ツールで「ネットワーク」タブを開きます（「ログを保持」がオンになっていることを確認します）。
1. 無効な値でフォームに入力します（ログインに失敗する必要があります）。
1. クリック **Submit**.
1. 「ネットワーク」タブから最初のリクエストを cURL としてコピーし、PHPSESSID と form_key の値を探します。
1. 更新された cURL リクエストの PHPSESSID と form_key の値を置き換えて送信します。
