...
* 手動でトリガーしたスナップショットは、7 日間保存されます。 保存期間を超えてスナップショットを保存する必要がある場合は、その期間内に別のフォルダまたは外部サーバにスナップショットをコピーします。 後日スナップショットをリストアする場合は、[ データベース・ダンプをサーバから直接インポートする ](https://experienceleague.adobe.com/en/docs/commerce-knowledge-base/kb/how-to/restore-a-db-snapshot-from-staging-or-production#meth3) と同じ手順に従います。
...
* 手動でトリガーしたスナップショットは、**7 日間** 保存されます。 保存期間を超えてスナップショットを保存する必要がある場合は、その期間内に別のフォルダまたは外部サーバにスナップショットをコピーします。 後日スナップショットをリストアする場合は、[ データベース・ダンプをサーバから直接インポートする ](https://experienceleague.adobe.com/en/docs/commerce-knowledge-base/kb/how-to/restore-a-db-snapshot-from-staging-or-production#meth3) と同じ手順に従います。
