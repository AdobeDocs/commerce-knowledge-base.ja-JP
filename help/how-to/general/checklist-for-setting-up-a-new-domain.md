...
これは、以前に `ece-tools` パッケージで `config:dump` コマンドを実行することで、ビルド時に [SCD](https://experienceleague.adobe.com/en/docs/commerce-on-cloud/user-guide/develop/deploy/static-content#setting-the-scd-on-build) を設定したことを意味します。
